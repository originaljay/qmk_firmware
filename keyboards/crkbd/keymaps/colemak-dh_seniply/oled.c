...
void render_layer_symbol(void) {
    ...
    } else if(layer_state_is(_GAMEEXT)) {
        oled_write_P(raise_layer, false);
...
void render_layer_name(void) {
    bool extend = layer_state_is(_EXTEND);
    bool gameext = layer_state_is(_GAMEEXT);
    bool symbols = layer_state_is(_SYMBOLS);
    bool function = layer_state_is(_FUNCTION);
    bool numbers = layer_state_is(_NUMBERS);
    bool gaming = layer_state_is(_GAMING);
